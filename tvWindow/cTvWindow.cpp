...
public:
	...
	void drawServices (ID2D1DeviceContext* dc, D2D1_SIZE_F client, IDWriteTextFormat* textFormat,
		...
		for (auto service : mServiceMap) {
			...
			dc->DrawText (std::wstring (str.begin(), str.end()).data(), (uint32_t)str.size(), textFormat, r, whiteBrush);
			r.top += 20.0f;
			}
		}
	//}}}
	...
	// vars
	cDecodeContext* mDecodeContexts[kMaxVidDecodes];
	...
	void decodeVidPes (cPidInfo* pidInfo) {
		...
		switch (pidInfo->mStreamType) {
			...
			case 2: {
				...
				while ((contextIndex < kMaxVidDecodes) && mDecodeContexts[contextIndex]) {
...
public:
	//{{{
	cTvWindow() {
		for (auto i = 0; i < kMaxVidDecodes; i++) {
			mBitmaps[i] = nullptr;
			mBitmapPts[i] = 0;
			}
		}
...
private:
	...
	void loader (wchar_t* wFileName) {
		...
		while (true) {
			...
			if (numberOfBytesRead) {
				...
				while (mPlayAudFrame < mTs.getLoadAudFrame() - (kMaxAudFrames-6))
					Sleep (1);
	...
	uint64_t mBitmapPts[kMaxVidDecodes];
	ID2D1Bitmap* mBitmaps[kMaxVidDecodes];
	//}}}
	};
