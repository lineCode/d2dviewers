...
static wchar_t wNetworkNameStr[20];

static tProgramMap mProgramMap;
static tPidInfoMap mPidInfoMap;
static tServiceMap mServiceMap;

static float mLargest = 10000.0f;
//}}}

//{{{
static void recordChange (int sid) {
...
//}}}
//{{{
static void updatePidInfo (int pid) {
...
//{{{
static void getDescrText (unsigned char* buf, int len, char* text) {
...
//}}}
//{{{
static void getDescrName (unsigned char* buf, int len, char* name) {
...
//}}}
//{{{
static void parseDescr (char* sectionName, int key, unsigned char* buf, int tid) {
...
//}}}
//{{{
static void parseDescrs (char* sectionName, int key, unsigned char* buf, int len, unsigned char tid) {
...
//}}}
//{{{
static void parsePat (unsigned char* buf, unsigned char* bufEnd) {
...
//}}}
//{{{
static void parseNit (unsigned char* buf, unsigned char* bufEnd) {
...
//}}}
//{{{
static void parseSdt (unsigned char* buf, unsigned char* bufEnd) {
...
//}}}
//{{{
static void parseEit (unsigned char* buf, unsigned char* bufEnd) {
...
//}}}
//{{{
static void parseTdt (unsigned char* buf, unsigned char* bufEnd) {
...
//}}}
//{{{
static void parsePmt (int pid, unsigned char* buf, unsigned char* bufEnd) {
...
//}}}
//{{{
static void parseSection (int pid, unsigned char* buf, unsigned char* bufEnd) {
...
void parsePackets (BYTE* samples, size_t sampleLen) {
  ...
  while (i < sampleLen) {
    if ((samples[i] == 0x47) && (((i + 188) >= sampleLen) || (samples[i+188] == 0x47))) {
      ...
      bool isSection = (pid == PID_PAT) || (pid == PID_SDT) || (pid == PID_EIT) ||
      ...
      if (isSection) {
        ...
        tServiceMap::iterator serviceIt = mServiceMap.find (pidInfoIt->second.mSid);
        if (serviceIt != mServiceMap.end()) {
          if (pid == serviceIt->second.getSubPid()) {
            //{{{  sub PES
          ...
          // should also write modified PMT,PAT,SDT selected EIT
          }
        }
...
void parseFile (wchar_t* filename) {
  ...
  HANDLE mReadFile = CreateFile (filename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);

  DWORD samplesLen = 0;
  do {
    ReadFile (mReadFile, &samples, 240*188, &samplesLen, NULL);
    if (samplesLen > 0)
      parsePackets (samples, samplesLen);
    } while (samplesLen > 0);
...
void parseFileMapped (wchar_t* filename) {
  ...
  HANDLE mReadFile = CreateFile (filename, GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
  size_t bytes_left = GetFileSize (mReadFile, NULL);
  HANDLE hMap = CreateFileMapping (mReadFile, NULL, PAGE_READONLY, 0, 0, NULL);
  BYTE* samples = (unsigned char*)MapViewOfFile (hMap, FILE_MAP_READ, 0, 0, 0);

  parsePackets (samples, bytes_left);
...
void renderPidInfo (ID2D1DeviceContext* d2dContext, D2D1_SIZE_U client,
  ...
  if (!mPidInfoMap.empty()) {
    // title
    wchar_t wStr[200];
    swprintf (wStr, 200, L"%ls %ls services:%d %4.1fmb %d/%d",
              getCurTime(), getNetworkName(), getNumServices(),
              (getNumPackets()*188)/pow(2,20), getNumPackets(), getNumDiscontinuity());
