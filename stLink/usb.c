...
ssize_t send_recv (struct stlink_libusb* handle, int terminate,
  ...
  int res = 0;
  int t = libusb_bulk_transfer(handle->usb_handle, handle->ep_req, txbuf, (int) txsize, &res, 3000);
  if (t) {
...
/*{{{*/
static inline int send_only (struct stlink_libusb* handle, int terminate, unsigned char* txbuf, size_t txsize) {
